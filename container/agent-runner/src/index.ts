    ...
    if (message.type === 'result') {
      ...
      const usage: UsageData | undefined = usageObj
        ? {
            ...
            cost_usd:
              typeof rm.total_cost_usd === 'number' ? rm.total_cost_usd : 0,
            duration_ms:
              typeof rm.duration_ms === 'number' ? rm.duration_ms : 0,
            num_turns: typeof rm.num_turns === 'number' ? rm.num_turns : 0,
          }
        : undefined;
      log(
        `Result #${resultCount}: subtype=${message.subtype}${textResult ? ` text=${textResult.slice(0, 200)}` : ''}${usage ? ` tokens=${usage.input_tokens}in/${usage.output_tokens}out cost=$${usage.cost_usd.toFixed(4)}` : ''}`,
      );
      writeOutput({
        status: 'success',
